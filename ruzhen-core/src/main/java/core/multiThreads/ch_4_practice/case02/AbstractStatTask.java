...
public abstract class AbstractStatTask implements Runnable{
    ...
    protected void report(Map<Long,DelayItem> summaryResult) {
        ...
        for(DelayItem delayItem: summaryResult.values()){
            sampleCount = delayItem.getSampleCount().get();
            ps.printf("%s%Sd%Sd%Sd%n", getUTCTimeStamp(delayItem.getTimeStamp()), delayItem.getTotalDelay().get()/sampleCount,sampleCount/sampleInterval, sampleCount);
        }
    }

    private String getUTCTimeStamp(long timeStamp) {
        calendar.setTimeInMillis(timeStamp);
        String temTs = sdf.format(calendar.getTime());
        return temTs;
    }
}
